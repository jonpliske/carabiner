...
void function () {
  ...
  module.exports = function () {
    ...
    monitorSearch = function (job, cb) {
      var count, jobUrl, roundWithPrecision;
      count = 0;
      jobUrl = options.scheme + '://' + options.host + ':' + options.port + job.qualifiedPath;
      roundWithPrecision = function (num, percision) {
        var multiplier;
      ...
      return async.until(function () {
        ...
        return job.fetch(function (err) {
          var cache$, doneProgress, eventCount;
          if (null != err)
            done(err);
          cache$ = job.properties();
          eventCount = cache$.eventCount;
          doneProgress = cache$.doneProgress;
          if (eventCount > count) {
            console.log('-- in progress, ' + eventCount + ' events, ' + roundWithPrecision(doneProgress * 100, 2) + '% complete');
            count = eventCount;
          }
          return done();
        });
